...
export function useRouterPush(inSetup = true) {
  ...
  function routerPushByPathWithMetaQuery(key: RouteKey) {
    const allRoutes = router.getRoutes();
    const currentRoute = allRoutes.find(item => item.name === key) || null;
    ...
    return routerPush({ path: currentRoute?.path, query });
  }
