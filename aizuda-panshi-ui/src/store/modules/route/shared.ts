...
function sortRouteByOrder(route: ElegantConstRoute) {
  if (route.children?.length) {
    ...
    route.children.sort((next, prev) => (Number(prev.meta?.order) || 0) - (Number(next.meta?.order) || 0));
...
export function sortRoutesByOrder(routes: ElegantConstRoute[]) {
  ...
  routes.sort((next, prev) => (Number(prev.meta?.order) || 0) - (Number(next.meta?.order) || 0));
